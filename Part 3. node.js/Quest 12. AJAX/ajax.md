...
* `middleware`: 개발자와 기반 시스템 간에 존재하는 소프트웨어. 시스템에 따라 운영체제를 의미하는 것일 수도 있고 Node에서 제공되는 것과 같은 기반기술 일 수도 있다. 애플리케이션과 기반 시스템 사이에서 통신 체인 역할을 수행하는 것이다. NodeJS에서는 request, response를 가공처리 하여 메소드로 넘겨주는 역할을 함.
