...
* 4계층(Transport) - 사용자가 보낸 데이터가 목적지에 정확하게 도착하도록 관리합니다(발신지 대 목적지(end to end) 간 제어와 에러 관리). 패킷 생성, 패킷 오류 관리를 담당합니다. 전송단위는 Segment, 주소는 Port를 사용합니다. 전송 계층을 기점으로 하위 계층으로 이루어진 사용자 서비스로 구별될 수 있다. 대표적인 프로토콜은 TCP이다.
